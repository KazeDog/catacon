...
class RxnCatNet(pl.LightningModule):
    def __init__(self,
        ...
        self.linear = nn.Sequential(nn.Linear(d_model * 2, d_model))
    ...
    def forward(self, mol_r, mol_p, mol_c, mol_class, label=None, log=False):
        ...
        mol_rea = torch.cat([mol_r, mol_p], dim=-1)  # (batch, d_model * 2)
        # mol_rea, _ = self.gru(sequence)  # 正确三维输入
        # mol_rea = mol_rea[:, -1, :]  # (batch, d_model)
        mol_rea = self.linear(mol_rea)
    ...
    # def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        # mol_r, mol_p, mol_c, label = batch
        # loss = self.forward(mol_r, mol_p, mol_c, label)
        # self.log('predict_loss', loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        # return {
        #     'predict_loss_cross': loss,
        # }
